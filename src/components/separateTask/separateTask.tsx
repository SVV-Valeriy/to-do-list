...
export const SeparateTask = () => {
    ...
    const subTasksElement = separateTask?.subTasks.map(({id, name, description, status, priority, date, comments}) => (
        <SubTask key={id} subId={id} name={name} description={description} status={status} priority={priority} date={date}
                 comments={comments}/>))
            ...
            <div className={style.taskWithMenu}>
                <div>
                    <div className={style.checkboxWithName}>
                        <CheckboxTask changeStatus={changeStatus} id={separateTask!.id}
                                      status={separateTask!.status}/>
                    {editMode ? <FormSeparateTask changeTask={changeTask} OffEditMode={OffEditMode} id={separateTask!.id}
                                                  description={separateTask!.description} name={separateTask!.name}/>
                        :
                    ...
                    <div className={style.subAndComment}>
                        ...
                        <>
                            ...
                            <Modal isOpenModal={isOpenModal} onClose={onClose}>
                                <FormForTask isOpen={isOpenModal} id={Number(id)} setOpenModal={setOpenModal}
                                             title={title}
                                             submit={addSubTask}
                ...
                <TaskMenu deleteTask={deleteTask} id={id} priority={separateTask!.priority} date={separateTask!.date}/>
            </div>
        </div>
    )
}